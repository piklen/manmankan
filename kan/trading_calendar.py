...
- 跨时区用户可通过 TZ 环境变量影响 datetime.now() · 或设 KAN_DATA_AVAIL_OFFSET_MIN (架-4)

v0.0.4.7 防御纵深 (架-2 + 安-1/2/3 + CR-3):
...
# 模块级 memo + 锁 (CR-3 double-checked locking)
...
def _sanity_check_dates(dates: set[date], context: str = "") -> bool:
    ...
    用于 (安-1) cache 内容校验 + (安-3) akshare 返回值校验。
    """
    ...
    if min_d.year >= SANITY_MAX_YEAR_MIN:
        ...
        return False
    today = _today()  # CR-5 v0.0.4.8: 用 kan._time.today() 集中 SoT
    max_d = max(dates)
...
def _read_cache() -> set[date] | None:
    ...
    安-1: 加 sanity check 三 invariant · 失败返 None 触发重拉。
    安-6: except 缩窄到 (JSONDecodeError | ValueError | OSError) + stderr warn。
    """
...
def _write_cache(dates: set[date]) -> None:
    ...
    安-2: chmod 失败不再静默 contextlib.suppress · 改 stderr warn。
    """
    ...
    # 安-2: chmod 0o600 后真校验 · 失败 stderr warn
    try:
        ...
        # 安-3 (v0.0.4.7 P0): 不暴露 errno 原文 · 仅给类型 · 防容器逃逸侦察辅助
        print(
...
def _fetch_from_akshare() -> set[date]:
    ...
    架-2: 失败抛 RuntimeError (由 get_trade_dates 兜底降级 weekday 启发式)。
    安-3: akshare 返回值零校验 sanity check (与 _read_cache 同结构)。
...
def _weekday_heuristic(as_of: datetime) -> date:
    ...
    架-2 fail-soft fallback · 不抛 RuntimeError。
    """
    today = as_of.date()
...
def get_trade_dates() -> set[date]:
    ...
    CR-3: double-checked locking 防 fetch_batch 多线程并发首调 akshare。
    架-2: akshare fail → 返回空 set · 调用方走 weekday 启发式 path · 不抛 RuntimeError。
    """
    ...
    with _memo_lock:
        ...
        try:
            ...
            # 架-2: 降级 · 不抛 · 返回空 set · 调用方走 weekday 启发式
            print(
...
def is_trading_day(d: date) -> bool:
    ...
    架-2 fail-soft: get_trade_dates() 空集合时退化 weekday 启发式。
    """
...
def latest_trade_date(as_of: datetime | None = None) -> date:
    ...
    架-2 fail-soft: 交易日历不可用时退化 weekday 启发式 · 不抛 RuntimeError。
    """
    if as_of is None:
        as_of = datetime.now()
    ...
    # 架-2 退化路径: trade_days 为空 (akshare + cache 双失败)
    if not trade_days:
    ...
    # 架-2: 14 天保护失败也降级 · 不抛 RuntimeError
    print(
...
def market_phase(as_of: datetime | None = None) -> str:
    ...
    架-2 fail-soft: trade_days 为空时用 weekday 启发式判 trading_day。
    """
    if as_of is None:
        as_of = datetime.now()
