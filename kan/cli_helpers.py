...
def _auto_fetch_stale(pairs: list[tuple[str, str]]) -> None:
    ...
    with console.status(
        ...
        # latest_trade_date 现已 fail-soft (v0.0.4.7 架-2) · 不抛 RuntimeError ·
    ...
    with Progress(
        ...
        # UX-4 (v0.0.4.8): 累计失败 symbol · GIL 保护 list.append 原生 thread-safe
        ...
        def _on_done(symbol: str, ok: bool, _err_msg: str | None) -> None:
            ...
            # UX-4 (v0.0.4.8): ✅/❌ emoji + 累计失败数
            # UX-2 (v0.0.4.8 finalize · P0-5): truncate name to 4 char + 紧凑 desc · 80 列不折行
