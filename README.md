...
> 排序:共振次数降序 → 各周期位置百分位(低点模式升序 / 高点模式降序)· 终端 < 130 列时自动选周期子集,共振列始终显示。
...
> 语法:`--pos PERIOD:OP:VAL`(PERIOD ∈ 3/5/7/10/15/30/60/90/120/180,OP ∈ lt/lte/gt/gte/eq/ne)· `--resonance low|high:OP:VAL` · `--exclude-st`。`kan find` 是**用户主导的条件筛选器**:规则由你显式指定,**无内置 preset、无评分、无评级、无推荐**,只返回符合你规则的股票。合规细则见 [`docs/compliance.md` §7](docs/compliance.md)。
