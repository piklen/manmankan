...
**TuShare Pro** 是付费 / 积分制 SaaS（[tushare.pro](https://tushare.pro)）。本功能为接入第一步 —— **仅日 K**，对齐现有 fetcher 能力。

### 目标

- 允许用户配置 **token + 端点** 两项后，让 TuShare Pro 顶替 baostock 作为 fetch_kline 主路径
...
- HTTP 请求体里的 token 仅传给用户配置的 endpoint；自定义端点是用户自主选择，不做额外网络代理配置
...
| 决策 | 选项 | 选了 | 理由 |
|---|---|---|---|
| 代理语义 | 端点替换 / HTTP proxy / 都做 | **端点替换** | "默认官方但可自定义"语义对齐端点；HTTP proxy 已经能通过系统 env var 实现 |
