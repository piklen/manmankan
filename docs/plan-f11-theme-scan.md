...
- 所有 `pytest` / `ruff` / `uv` 命令在 worktree 根目录运行(`/Library/Code/AI/***REMOVED***/manmankan/.worktrees/feat-v0.0.5-f11-theme`)。
...
**测试纪律:**
- 沿 v0.0.4.8 ***REMOVED*** LOCKED "CliRunner runtime 真测 · 禁 bootstrap 字符串作弊"。
...
dependencies = [
    "akshare>=1.14,<2",
    "pandas>=2.0,<3",  # v0.0.4.8 P0-3 (***REMOVED***): 收紧 <3 (...)
    "pydantic>=2.0,<3",
    "rich>=13.0,<16",
    "typer>=0.12,<0.26",
    "pyarrow>=15.0,<25",
...
```toml
    "numpy>=1.26,<3",
    "adata>=2.9,<3",  # ***REMOVED***: 题材位置扫描 · 同花顺 catalog/成分股 + 东财 K 线/反查 · 2026-05-23 spike LOCKED
]
```
...
## Task 4: `get_theme_constituents` · THS 优先 + EM fallback + ***REMOVED***
...
from kan import cli_theme_cmds   # noqa: F401  · register `kan theme` (***REMOVED***)
```
...
git commit -m "feat(theme): kan theme list/search sub-command tree · ***REMOVED*** 发现入口"
```
...
**Files:**
- Modify: 全部 ***REMOVED*** 相关 .py 文件(若 grep 命中红线词)
...
```python
"""AGENTS.md §6 红线词 audit · grep ***REMOVED*** 相关源码 · 防回归。"""
import re
from pathlib import Path

# 红线词:不允许出现在 ***REMOVED*** 主体代码(disclaimer 第三人称语境例外)
***REMOVED***_SOURCE_FILES = [
    "kan/boards.py",
...
def test_no_redline_words_in_f11_source():
    """硬红线词不应出现在 ***REMOVED*** 任何主体代码中。"""
    repo = Path(__file__).parent.parent
    violations = []
    for relpath in ***REMOVED***_SOURCE_FILES:
        p = repo / relpath
        if not p.exists():
            continue
...
Expected: 525 (baseline) + 60 (新加 ***REMOVED*** case) ≈ `585 passed`(网络 6 个 deselect 不计)。
...
- [ ] **Step 1: CHANGELOG.md 加 v0.0.5.0 ***REMOVED*** 条目**
...
- 🆕 数据源:**adata**(同花顺 catalog/成分股 + 东方财富 datacenter K 线/反查 + EM push2 成分股 fallback 走 ***REMOVED*** 5min cooldown)。零 token 零配置 · 跟 `--industry` 申万源平级。
...
Expected: `585+ passed`(525 baseline + ~60 新 ***REMOVED*** case · 网络跳)。
...
- [ ] CHANGELOG.md 含 ***REMOVED*** Added 条目
...
git commit -m "docs(theme): ***REMOVED*** 题材位置扫描 · CHANGELOG v0.0.5.0 entry"
git push -u origin feat/v0.0.5-f11-theme
```

预期:推送成功 · GitHub 显示 PR-ready 状态。***REMOVED***后续:

1. 开 PR 把 `feat/v0.0.5-f11-theme` → `feat/v0.0.5.0`(扩 PR #18 或开新 PR)
2. 触发 7 角色 ***REMOVED*** v1.0 第 5 次实施(主会话 + 6 sub-agent)
...
- ✅ ***REMOVED*** 全 11 命令矩阵 + `kan theme list/search` 上线
- ✅ adata 各取***REMOVED***数据源(catalog/成分股 THS · K线/反查 EM datacenter · EM 成分股 fallback)
- ✅ 测试 baseline 525 → 585+
...
**完成后清理**(plan 实施完毕后,***REMOVED***定):

- `***REMOVED***manmankan-v0.0.5/v0.0.5.0/***REMOVED***-theme-scan.md`(v3 LOCKED 设计稿 · § 4 已被 supersede)→ ***REMOVED***定保留 / 移到 archive / 删
...
- "top N 活跃热度榜"(本版静态拼音序 · 留 ***REMOVED***.2 · 需 O(391) HTTP + cron + 24h cache + 反爬观察期)
