...
- 人读终端按方向分组、同档各周期相邻；单股和批量都保留全部请求档位与条件结果。解释比例时必须同时给分子/分母：`1/1` 不能只改写为 `100%`，零触及不是零概率。
- `--down 3 --up 7` 只复核用户明确给出的 `-3%` 下探与 `+7%` 上冲，不代表工具替用户选择阈值。
- 指定幅度在终端优先用文字回答，然后才展示档位表；`close_positive` 是相对前收上涨，不是持仓盈利。短窗口包含在长窗口内，不能把两窗口视作独立验证。
