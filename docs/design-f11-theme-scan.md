...
| EM 成分股(fallback) | `info.concept_constituent_east(concept_code=)` | `push2.eastmoney.com` | ⚠️ 首 1-2 次通 · stress 后 ConnectionError | 反爬触发 · 走 ***REMOVED***器 5min cooldown |
...
- EM 成分股仅作 fallback:`get_theme_constituents` 优先调 THS · THS 失败 → 检查 ***REMOVED***器 EM 是否冷却中 · 否则尝试 EM · EM 失败标记 down 5min。
...
| `get_theme_constituents(theme: Theme, force: bool = False) -> list[tuple[str, str]]` | THS 优先 → ***REMOVED***器检查 EM 是否冷却 → EM fallback · per-theme JSON cache 24h TTL |
...
题材线 disclaimer 比 `--industry` 多一档 —— "题材跟风风险高于行业" 是 ***REMOVED*** 独有警示。
