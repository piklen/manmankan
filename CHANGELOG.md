...
- `kan fetch --all` 改为按交易日批量拉取全市场截面，再以 12 路网络并发、最高 32 路缓存写入物化逐股 K 线；不再为约 5500 只股票逐只请求。新增 `--workers/-j`（1-32）显式并发上限，普通逐股 TuShare 路径的自适应上限同步提高到 32。
- 全市场刷新不再同时保留完整日截面与 5500 份逐股结果，也跳过仅为拆分缓存服务的 200 万行全表排序。真实 5534 只、360 交易日、强制刷新两轮耗时 286-393 秒（旧路径实测 4839 秒），成功率 5534/5534；峰值内存由 1.34 GB 降至约 1.00 GB。
...
- 修复 TuShare/PanShu 单次网络抖动会打开 5 分钟全局熔断，导致剩余数千只股票静默退化到单线程 Baostock 的问题；可重试错误现在短退避后继续，批量源整体不可用时快速明确失败，不再制造一小时级串行长尾。
- 修复最新交易日 `stk_factor_pro` generation 尚未完整时整批失败：仅在最新交易日使用已通过全市场完整性校验的 `daily` 截面补齐；历史交易日仍严格保持前复权口径。
- 修复有效的北交所 920 段在慢速降级链路中被误报无数据；真实全市场基准覆盖 331/331 只 920 股票。
- 修复 Web 全市场筛选和详情事实只读取默认自选分组、对比接口不接受 `SH600519` / `600519.SH` 等常见代码格式，以及 AND / OR 单选框缺少同组语义的问题。
