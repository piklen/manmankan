...
> 7 角色 release-review 第 7 次实施 · 49 finding · 9 P0 全修 (审计档案在维护者私有总部仓)
...
- `kan/_log.py` 统一 debug_log helper · `KAN_DEBUG` env var 控制可见 · 21 处 broad except 加诊断 (CR-4)
- `kan/_time.py` `today()` SoT helper · 集中 `datetime.now().date()` 3 处避免漂移 (CR-5)
- fetched_at 凌晨/晚间日界提示 · 当天 00:00-04:59 显示 "今晨 01:00" · 昨天 22:00-23:59 显示 "昨晚 23:50" (UX-3)
- spinner ✅/❌ emoji + 累计失败数 · `⏳ 补数据 169 只 · ✅ 茅台` / `❌ 中铝… · 失败 3` 80 列不折行 (UX-4 · 含 v0.0.4.8 finalize UX-2 视觉宽收紧)
- `kan add` 无参中文友好 · 不再 typer 英文 "Missing argument 'SYMBOLS...'" (U-2)
- `kan remove` / `kan info` 同款散户中文 · 兑现 U-2 承诺到全命令组 (U-1)
- 错误消息加"下一步引导" · `未找到... · 试 kan list 看自选 / 用代码精确加 kan add 600519` (U-3)
- 命令速记表加版本号 · `慢慢看 · v0.0.4.8 · 命令速记` (UX-5)
- install.sh / install.ps1 SHA256 在 release notes 公布 (安-4 短期 · 文化层防御)

### Changed

- 涨跌停状态警告改纯状态描述 · 删 "可能回落/可能回升/都是正常波动" 预测性词 (PM-4 + UX-1 cross-validated AGENTS.md §6 红线)
- README 新手专区 default open + Verify before bash 段套 `<details>` 折叠 · 真小白默认不见 Verify (B.4 + B.5 UX-4)
- 21 处 `except Exception` 加 `debug_log` 调用 (15 silent path · 6 已 user-visible 不改)
- 收紧 `pandas>=2.0,<3` 防 pandas 3.0 read_parquet schema 严格化破坏 (架-5 maintainer Option C)
- `debug_log` best-effort redact `/Users/<user>` + `token=<redacted>` 防 issue 截图 PII leak (安-2)

### Fixed

- bootstrap-test 作弊 7 处 → CliRunner runtime 真测 (CR-1 LOCKED 5-12)
...
- status.md auto-check hook 落地 (跨子项目治理 · 合-1 欠 3 版上限触顶 hard gate)
- v0.0.4.7 commit body 隐私 leak rebase reword (8 commits) + force-push main + retag v0.0.4.7 / v0.0.4.7.1 (安-1 LOCKED 闭环)
- 7 角色 release-review v1.0 第 4 次实施 · 45 finding · 8 P0 (62% 单角色独家)
- ADR-0016 中性化 SELF_EXCLUDES 加 `projects/<name>/reviews/` (合法包含 finding 字面值)

### Deferred to v0.0.4.9 / v0.0.5.0

- v0.0.4.8.1 hotfix: install.sh / install.ps1 散户化文案 5 项 (PM-3 LOCKED 分发资产独立 patch)
- v0.0.4.9: 子命令 --help 信息密度提升 (U-2 sub-agent 自降级) + 安-3 pip-audit CI step + 安-5 rebase-note 内部审计
...
- **依赖版本范围加严**：typer / rich / pandas / numpy / pydantic / akshare / pyarrow / baostock
  ...
  v0.0.4.3 装机崩溃就是这个原因（用户拿到 pandas 3.0 / typer 0.25 / rich 15
  这些跟当前代码不兼容的版本）。
...
- **release.yml 加 post-publish smoke matrix**：发版后 sleep 60s 等 PyPI CDN 同步，
  ...
  等核心命令，验证 entry point + import chain。**直接挡住 v0.0.4.3 类装机崩 ship**。
...
- README 第 14 行版本横幅同步到 v0.0.4.4（v0.0.4.3 因装机崩溃已 yank）。
