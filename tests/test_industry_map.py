...
class TestFetchSwL1Map:
    ...
    def test_no_token_empty(self, temp_env, monkeypatch):
        monkeypatch.setattr("kan.data.industry_map._fetch_tushare_sw_l1_members", lambda: None)
    ...
    def test_builds_and_caches(self, temp_env, monkeypatch):
        ...
        monkeypatch.setattr(
            "kan.data.industry_map._fetch_tushare_sw_l1_members", lambda: df.copy(),
        )
    ...
    def test_cache_hit_skips_fetch(self, temp_env, monkeypatch):
        ...
        def _f():
            calls["n"] += 1
            return df.copy()
        monkeypatch.setattr("kan.data.industry_map._fetch_tushare_sw_l1_members", _f)
