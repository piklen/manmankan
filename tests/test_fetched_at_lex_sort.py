"""***REMOVED*** v0.0.4.7 · 补测试覆盖盲区.

部分 finding (***REMOVED*** 全套 3 类) 已被其他 test 覆盖:
- warning / title runtime 行为 → tests/test_trend_cli.py CliRunner 真测 (***REMOVED*** v0.0.4.8 改造完成)
...
- test_fetched_at_lex_sort_equals_time_sort (验"取最新" lex 排序语义 ·
  ***REMOVED*** v0.0.4.5 的 main-session fix 配套 · 防回归)

***REMOVED*** 其他 2 项 (future_date with warning · scan/info 命令组完整覆盖)
推 v0.0.4.8 ***REMOVED*** (CLI 命令组覆盖率 6% → 60%+).
"""
...
def test_fetched_at_lex_sort_equals_time_sort():
    ...
    ***REMOVED*** v0.0.4.5 fix: `if t and (fetched_at is None or t > fetched_at): fetched_at = t`
