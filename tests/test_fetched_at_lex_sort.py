"""ISO timestamp lex 排序 == 时间排序 invariant 防回归(补的覆盖盲区).

部分 finding 已被其他 test 覆盖:
- warning / title runtime 行为 → tests/test_trend_cli.py CliRunner 真测(历史背景完成)
...
- test_fetched_at_lex_sort_equals_time_sort(验"取最新" lex 排序语义 ·
  历史背景的 main-session fix 配套 · 防回归)

其他 2 项(future_date with warning · scan/info 命令组完整覆盖)推 历史背景(CLI 命令组覆盖率 6% → 60%+)。
"""
...
def test_fetched_at_lex_sort_equals_time_sort():
    ...
    历史背景fix: `if t and (fetched_at is None or t > fetched_at): fetched_at = t`
    ...
    times = [
        ...
        "2026-05-12 23:59",  # 昨晚
        "2026-05-13 02:55",  # 凌晨拉数据 (历史背景bug 场景)
    ]
    # Lex sort
