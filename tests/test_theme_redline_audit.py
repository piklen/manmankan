...
def test_theme_disclaimer_4_lines_present():
    """render/theme.py 必须含 4 行 disclaimer 关键短语。"""
    p = Path(__file__).parent.parent / "kan/render/theme.py"
    content = p.read_text(encoding="utf-8")
...
def test_theme_cmds_have_education_disclaimer():
    """cli/theme_cmds.py 必须含散户教育 disclaimer(UX-4 后 disclaimer 改 SOT · const import)。"""
    p = Path(__file__).parent.parent / "kan/cli/theme_cmds.py"
    content = p.read_text(encoding="utf-8")
    # UX-4 后:THEME_VS_INDUSTRY / THEME_CLASSIFICATION / THEME_RISK 三个 const 任一引用
