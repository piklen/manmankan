...
# ════════════════════════════════════════════════════════════════
# ***REMOVED*** + ***REMOVED*** + ***REMOVED*** stale/intraday warning runtime 真测
# (***REMOVED*** v0.0.4.8 改造自 test_cli_helpers_format.py TestNoLegacyTextInWarnings grep-source 作弊)
# ════════════════════════════════════════════════════════════════
def test_trend_stale_warning_uses_new_phrasing(
    runner: CliRunner, monkeypatch: pytest.MonkeyPatch
) -> None:
    """***REMOVED*** + U-5 真测: stale 警告应含'当前缓存到 X 收盘' + '数据滞后 N 天' · 不再'应有最近交易日'.

    ***REMOVED*** v0.0.4.8: 旧 grep-source 作弊改 CliRunner 真测 ·
...
def test_trend_intraday_warning_compliant_phrasing(
    ...
    """***REMOVED*** + ***REMOVED*** 真测: 盘中警告应是状态描述 · 不含预测性'下一秒打开' AGENTS.md §6 红线词.

    ***REMOVED*** v0.0.4.8: 旧 grep-source 改 CliRunner 真测 ·
    ...
    assert result.exit_code == 0
    output = result.output
    # 新文案 v0.0.4.8 (***REMOVED***+***REMOVED*** cross-validated): 纯状态描述 · 移除 "可能回落/可能回升/都是正常波动" 预测性词
    ...
    assert "下一秒打开" not in output, "预测性词 '下一秒打开' 应删除 (***REMOVED*** + ***REMOVED*** v0.0.4.7)"
    assert "都是正常波动" not in output, "v0.0.4.8 ***REMOVED***: '都是正常波动' 含预测语义 · 应删除"
    assert "可能回落" not in output, "v0.0.4.8 ***REMOVED***: '可能回落' 含方向词 · 应删除"


def test_trend_warnings_mutex_stale_wins(
    runner: CliRunner, monkeypatch: pytest.MonkeyPatch
) -> None:
    """***REMOVED*** 真测: stale+intraday 同时为 True 时只显示 stale · intraday 不显示 (if/elif 互斥).
    ...
    ***REMOVED*** v0.0.4.8: 旧 grep-source 作弊改 CliRunner 真测 ·
    ...
    assert "涨跌停标签反映当前时刻" not in output, (
        "stale=True 时不应同时显示 intraday 警告 (***REMOVED*** 互斥 · 用户首动作 fetch)"
    )
