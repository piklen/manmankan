"""D-2 + D-3 (v0.0.4.7 · ***REMOVED*** 5-13 反馈): _auto_max_workers 启发式测试.
...
教育性 (***REMOVED*** 5-13 反馈"并发可以根据系统的核数-1来自动适配"):
...
class TestKanWorkersEnvVar:
    ...
    def test_env_var_3_overrides_to_3(self, monkeypatch):
        ...
        with patch("os.cpu_count", return_value=8):
            assert _auto_max_workers() == 3

    def test_env_var_zero_falls_back(self, monkeypatch):
        """0 不在 1-50 范围 · 回退默认."""
        monkeypatch.setenv("KAN_WORKERS", "0")
        with patch("os.cpu_count", return_value=8):
            assert _auto_max_workers() == 12  # 8*2 cap 12

    def test_env_var_over_50_falls_back(self, monkeypatch):
        """100 超 50 上限 · 回退默认."""
        monkeypatch.setenv("KAN_WORKERS", "100")
        with patch("os.cpu_count", return_value=8):
            assert _auto_max_workers() == 12
...
class TestD1MigrationTextRemoved:
    ...
    def test_concurrency_message_uses_auto_workers_not_hardcoded_5(self):
        ...
        assert "并发 5" not in src, (
            "硬编码 '并发 5' 应替换为 _auto_max_workers() 动态值 (D-2)"
        )
        assert "_auto_max_workers" in src, (
            "应调用 _auto_max_workers 显示实际并发数 (D-2)"
        )
