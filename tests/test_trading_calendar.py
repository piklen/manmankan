"""trading_calendar.py 6 case production 故障模式覆盖 (***REMOVED*** + ***REMOVED*** + ***REMOVED***/2/3/6 + ***REMOVED***).
...
# ─────────────── Case 1: akshare raise (***REMOVED*** fail-soft) ───────────────
def test_akshare_raise_falls_back_to_empty_set(tmp_cache, capsys, monkeypatch):
    """***REMOVED***: akshare 抛 RuntimeError → get_trade_dates 返空 set · 不再 raise."""
    monkeypatch.setattr(
...
def test_latest_trade_date_uses_weekday_heuristic_when_empty(
    ...
    """***REMOVED***: trade_days 空时 latest_trade_date 退化 weekday 启发式 · 不抛 RuntimeError."""
    monkeypatch.setattr(
        tc, "_fetch_from_akshare",
...
# ─────────────── Case 2: akshare 返空 DataFrame (***REMOVED***) ───────────────
def test_akshare_returns_empty_dataframe_triggers_failure(monkeypatch):
    """***REMOVED***: akshare 返空 DataFrame 应被 _fetch_from_akshare 转 RuntimeError."""
...
# ─────────────── Case 3: akshare 返脏 (sanity 失败 · ***REMOVED***) ───────────────
def test_akshare_returns_dirty_data_triggers_sanity_failure(monkeypatch, capsys):
    """***REMOVED***: akshare 返回值 count 过少 (5 个 dates) 应触发 sanity check 失败."""
...
# ─────────────── Case 4: cache 损坏 (JSONDecodeError · ***REMOVED***) ───────────────
def test_corrupt_cache_triggers_refetch(tmp_cache, capsys):
    """***REMOVED***: cache JSONDecodeError 应被缩 except 范围 + stderr warn + 返 None."""
...
# ─────────────── Case 6: chmod 失败 → stderr warn (***REMOVED***) ───────────────
def test_chmod_failure_warns_but_does_not_crash(tmp_cache, capsys, monkeypatch):
    """***REMOVED***: chmod 0o600 失败不再静默 · stderr warn + 继续 (不抛)."""
    valid_dates = _make_valid_dates()
    ...
    def mock_chmod(self, mode):
        ...
        if self.name == "trade_dates.json":
            raise OSError("Operation not permitted (mock for ***REMOVED*** test)")
...
class TestSanityCheck:
    """***REMOVED*** 三 invariant 边界 case · 不依赖 akshare / file."""
