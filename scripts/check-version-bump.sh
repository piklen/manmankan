...
#
# 规则 (manmankan 项目 PEP 440 数字 release segment A.B.C[.D...]):
# - baseline = pyproject.toml 当前 version (例 0.0.6.9.1)
# - 前 3 段 A.B.C 是稳定段 (epoch.major.minor)
# - 第 4 段及之后是 patch tail · 由维护者明确指定
...
# 解析 baseline 前 3 段 (稳定段) + patch tail
# 例 0.0.6.9.1 → A=0 B=0 C=6 patch_tail=9.1
# 例 0.0.6     → A=0 B=0 C=6 patch_tail=0 (隐式)
IFS='.' read -r BASELINE_A BASELINE_B BASELINE_C BASELINE_PATCH_TAIL <<< "$BASELINE_VERSION"
BASELINE_PATCH_TAIL="${BASELINE_PATCH_TAIL:-0}"
...
FOUND_VERSIONS=$(echo "$COMMIT_MSG" | grep -oE 'v?[0-9]+(\.[0-9]+){2,}' | sort -u || true)
...
NEXT_PATCH=$(python3 - "$BASELINE_VERSION" <<'PY'
import sys
...
echo "   允许范围: ${BASELINE_STABLE}.* (patch tail 由维护者指定)" >&2
echo "   候选下一 patch:       ${NEXT_PATCH}" >&2
echo "" >&2
